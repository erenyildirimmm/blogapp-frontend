...
const BookPreview = ({
  ...
  return (
    <div className="bg-gray-900 rounded-lg m-4 relative hover:shadow-xl">
      <span className="py-1 px-2 absolute top-2 right-2 text-sm bg-primary text-white rounded-md ">
      ...
      <div className="px-4 py-2 mb-3">
        <div className="mb-4">
          <h4 className="my-2 inline-block text-white text-lg border-b-2 pb-1 border-primary">
            {title ? title : "Kitap İsmi"}
          </h4>
        </div>
        <div className="flex justify-start items-center gap-x-2 mb-3">
          <BiUserCircle className="text-primary text-2xl" />
          <h2 className="my-1 text-white">{author ? author : "Yazar"}</h2>
        </div>
        <div className="flex justify-start items-start gap-x-2">
          <PiBookOpenTextLight className="text-primary text-2xl" />
      ...
      <div className="bg-primary p-2 rounded-br-md rounded-bl-md">
        {userName}
      </div>
    </div>
  );
